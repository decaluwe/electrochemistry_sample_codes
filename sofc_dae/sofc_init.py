...
SV_0_CL_an = np.tile(np.hstack((np.array([phi_an_0 - phi_elyte_0]),C_k_an_0)),
    npoints_CL)
SV_0_elyte = np.array([phi_elyte_0, phi_elyte_0])
SV_0_ca = np.array([phi_ca_0 - phi_elyte_0, phi_ca_0])
SV_0 = np.hstack((SV_0_GDL_an, SV_0_CL_an, SV_0_ca))

# Create initial array of time-derivatives
...
class pars:
    ...
    eps_g_dy_Inv_AL = 1/dy_GDL/eps_gas_GDL
    ...
    # Ni surface area per unit geometric area:
    A_fac_Ni = 0.5*eps_solid_CL*3.*dy_CL*Pt_surf_frac/d_part_CL
    # Geometric area per unit double layer area:
    A_fac_dl = Pt_surf_frac/A_fac_Ni
    ...
    # Electrolyte conductivity, divided by thickness (S/m2)
    sigma_dy_elyte = sigma_elyte/dy_elyte
...
class ptr:
    ...
    # Cathode electric potential is just after phi_dl_ca:
    phi_ca = phi_dl_ca + 1

# Add indices for algebraic variables to pars
pars.algvars = np.array([ptr.phi_ca])
